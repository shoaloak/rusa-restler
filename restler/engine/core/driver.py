...
from checkers import CheckerBase
...
from engine.core.feedback_queue import ZmqClient
...
def extend(seq_collection: List[sequences.Sequence], fuzzing_requests, lock, random_gen):
    ...
    @return: The list of newly extended sequences.
    @rtype : List
    ...
    # Rusa filtering based on distance
    # Algorithm:
    # - if no distance, leave it be
	# - if some have distance, filter on lowest
    if Settings().dynamic:
        lowest_distance = None
        # Create new sequence collection with only sequences that have a distance
        # prevents ValueError: min() arg is an empty sequence error
        seq_w_dist_collection = [seq for seq in seq_collection if any(req.distance is not None for req in seq)]
        if seq_w_dist_collection:
            lowest_distance = min(req.distance for seq in seq_collection for req in seq)
            # print(f"previous size: {len(seq_collection)}")
            seq_collection = [seq for seq in seq_w_dist_collection if any(req.distance == lowest_distance for req in seq)]
            # update seq collection length 
            prev_len = len(seq_collection)
            # print(f"new size: {len(seq_collection)}")

    # loop through all possible endpoint requests
    for req in fuzzing_requests:
        for i in range(prev_len):
            seq: sequences.Sequence = seq_collection[i]
    ...
    # prune sequences based on coverage
    # if Settings().dynamic:
    #     # if there was any hit
    #     if any(seq.distance for seq in seq_collection[prev_len:]):
    #         # filter out sequences that did hit
    #         # TODO@: filter based on distance heuristic
    #         seq_collection = [seq for seq in seq_collection[prev_len:] if seq.distance]
    #         return seq_collection, extended_requests
...
def render_one_sequence(seq_to_render: sequences.Sequence, ith: int, checkers, generation: int, global_lock,
                        garbage_collector) -> List[sequences.Sequence]:
    ...
    @param global_lock: Lock object used for sync of more than one fuzzing jobs.
    @type  global_lock: multiprocessing.Lock object
    ...
    render_one_sequence.last_memory_consumption_check = getattr(render_one_sequence, 'last_memory_consumption_check', int(time.time()))

    if int(time.time()) - render_one_sequence.last_memory_consumption_check > (n_minutes * 60):
        logger.print_memory_consumption(GrammarRequestCollection(), Monitor(), Settings().fuzzing_mode, generation)
        render_one_sequence.last_memory_consumption_check = int(time.time())
    ...
    while True:
        ...
        renderings: sequences.RenderedSequence = current_seq.render(candidate_values_pool, global_lock, generation)
        ...
        # i.e. a primitive type fit the Request
        if renderings.valid:
            break
    ...
    # bfs needs to be exhaustive to provide full grammar coverage
        ...
        while renderings.sequence is not None:
            if renderings.valid:
                valid_renderings.append(renderings.sequence)
            renderings = current_seq.render(candidate_values_pool, global_lock, generation)
            apply_checkers(checkers, renderings, global_lock)
...
def render_parallel(seq_collection, fuzzing_pool, checkers, generation, global_lock, garbage_collector):
    ...
    """
    prev_len = len(seq_collection)
    result = fuzzing_pool.starmap(render_one_sequence,
...
def render_sequential(seq_collection: list[sequences.Sequence], fuzzing_pool, checkers, generation, global_lock,
                      garbage_collector):
    ...
    prev_len = len(seq_collection)
    for ith in range(prev_len):
        logger.write_to_main(f"{formatting.timestamp()}|render_sequential: Rendering sequence {ith+1} of {prev_len}")
        valid_renderings: List[sequences.Sequence] =\
            render_one_sequence(seq_collection[ith], ith, checkers, generation, global_lock, garbage_collector)
        # Extend collection by adding all valid renderings
        seq_collection.extend(valid_renderings)

    new_seq_collection = seq_collection[prev_len:]

    if len(new_seq_collection) == 0:
    ...
    for current_seq in seq_collection:
        ...
        for ith in range(len(sequences_to_render)):
            ...
            for prefix_len in range(rendered_prefix_length + 1, sequence_to_render.length + 1):
                ...
                valid_renderings = render_one_sequence(prefix_seq_to_render, sequences_count, checkers, generation, global_lock, garbage_collector)
...
def generate_sequences(fuzzing_requests, checkers, fuzzing_jobs=1, garbage_collector=None):
    ...
    @param garbage_collector: The garbage collector
    ...
    # only broken by timeout OR if BFS-esque algorithm
    while not should_stop:
        ...
        for length in range(min_len, max_len):
            # we can set this without locking, since no-one else writes (main
            # driver is single-threaded) and every potential worker will just
            # read-access this value.
            generation = length + 1
            fuzzing_mode = fuzzing_schedule[length] # always the same (for now)
            ...
            if specific_target_sequences is not None and seq_constraints_by_generation[generation]:
                ...
                # per generation, extend sequence collection with new requests (whose dependencies are satisfied)
                seq_collection, extended_requests = extend(seq_collection,
                                                           fuzzing_requests,
                                                           global_lock, random_gen)
                # seq_collection went from len4 to len1.
                # extended_requests is an empty list
                #print('@'*80)
                #print(len(seq_collection))
                #print(len(extended_requests))
            ...
            try:
                ...
                if not Settings().in_scenario_replay_mode() and specific_target_sequences is not None and \
                    ...
                    seq_collection = render_sequential(seq_collection, fuzzing_pool, checkers, generation, global_lock, garbage_collector)
            ...
            logger.write_to_main(
                f"{formatting.timestamp()}: END Generation: {generation} / "
                f"Sequences Collection Size: {len(seq_collection)} "
