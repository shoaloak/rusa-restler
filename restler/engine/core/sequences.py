...
from engine.core.requests import GrammarRequestCollection, Request
...
class RenderedSequence(object):
    ...
    def __init__(self, sequence=None, valid: bool = False, failure_info=None, final_request_response=None,
                 response_datetime=None, distance: int = None):
...
class Sequence(object):
    ...
    def __init__(self, request: Request = None):
        ...
        self.seq_i = 0
        self.requests = list(request) if request is not None else []
    ...
    def __str__(self):
        # TODO: make an elaborate tostring method s.t. we can view the sequences and their differences
        data = dict()
        data['seq_i'] = self.seq_i
        data['requests'] = self.requests
        data['_sent_request_data_list'] = self._sent_request_data_list
        data['rendered_prefix_status'] = self.rendered_prefix_status
        data['create_prefix_once'] = self.create_prefix_once
        data['re_render_prefix_on_succes'] = self.re_render_prefix_on_success
        data['executed_requests_count'] = self.executed_requests_count
        data['_used_cached_prefix'] = self._used_cached_prefix
        return str(data)
    ...
    def send_rendered_request(self, request, rendered_data, parser, tracked_parameters, updated_writer_variables,
        ...
        # logger.write_to_main(f'{formatting.timestamp()}|render: sending request {request.request_id[:6]} part of sequence gen{generation}, iter{self.seq_i}')
        # distance property is set.
    ...
    def render(self, candidate_values_pool, lock, preprocessing=False, postprocessing=False) -> RenderedSequence:
        ...
        request: Request = self.last_request
        ...
        for rendered_data, parser, tracked_parameters, updated_writer_variables, replay_blocks in\
            ...
            # Rusa feedback
            # We store distance and filter during extend()
            if Settings().dynamic:
                if response.distance is not None:
                    # Observed distance!
                    # self.distance.append(response.distance)
                    request.distance = response.distance
                else:
                    # Nothing, set max
                    # self.distance.append(sys.maxsize)
                    request.distance = sys.maxsize
            ...
            if not response.status_code:
                duplicate = copy_self()
                return RenderedSequence(duplicate, valid=False, failure_info=FailureInformation.MISSING_STATUS_CODE,
            ...
            duplicate: Sequence = copy.deepcopy(self)
            if lock is not None:
                lock.release()
