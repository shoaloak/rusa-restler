...
def send_request_data(rendered_data, req_timeout_sec=None, reconnect=None, http_sock=None) -> HttpResponse:
    ...
    while num_retries < MAX_RETRIES:
        ...
        req_identifier = None
        if Settings().dynamic:
            req_identifier = str(current_microseconds())
            # SYNC with the instrumentation
            ZmqClient().send(req_identifier)
            # ACK
            zmq_response = ZmqClient().recv_all()
            if zmq_response != ['{}']:
                # Data returned is considered an error.
                _RAW_LOGGING("Error: Desynchronized instrumentation feedback.")
                continue

        success, response = main_sock.sendRecv(rendered_data, req_timeout_sec, reconnect=reconnect)
        ...
        if Settings().dynamic:
            ...
            ZmqClient().send(req_identifier)
            # Receive distance
            zmq_response = ZmqClient().recv_json()

            # DEBUG
            # print('Rusa response: ' + str(zmq_response))
            # print('received http respons: \n\n' + response.to_str)

            if any(zmq_response):
                # Store lowest seen Feedback distance, since all feedback hooks return values
                response.distance = min(item['distance'] for item in zmq_response)
                # print('Rusa distance: ' + response.distance)
...
def call_response_parser(parser, response: Union[HttpResponse, None], request=None, responses=None):
