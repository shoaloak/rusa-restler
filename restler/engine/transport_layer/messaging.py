...
class HttpSock(object):
    ...
    def set_up_connection(self):
        try:
            ...
            # If proxy is enabled it becomes our new target
            if Settings().proxy:
                target_ip = Settings().proxy_host
                target_port = Settings().proxy_port
    ...
    def sendRecv(self, message, req_timeout_sec, reconnect=False) -> tuple[bool, HttpResponse]:
    ...
    def _sendRequest(self, message):
        ...
        if Settings().proxy:
            ...
            target_ip = self.connection_settings.target_ip or host
            target_port = self.connection_settings.target_port
            target = target_ip if target_port == 80 else f'{target_ip}:{target_port}'

            method, http_data = message.split('/', 1)
            message = f'{method} http://{target}/{http_data}'
...
def verify_proxy():
    ...
    host = Settings().host
    port = Settings().connection_settings.target_port

    proxy_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    try:
        proxy_socket.connect((Settings().proxy_host, Settings().proxy_port))
        proxy_socket.settimeout(PROXY_TIMEOUT)  # Set a timeout of 1 second, since we expect a localhost to be fast

        proxy_socket.send(
            f'CONNECT {host}:{port} HTTP/1.1\r\nHost: {host}\r\n\r\n'
            .encode())
        proxy_response = proxy_socket.recv(4096)
        if '200 Connection established' not in proxy_response.decode():
            raise OSError("Failed to connect to host using proxy")

        # everything good, close the connection
        proxy_socket.close()
        return True
    except ConnectionRefusedError:
        print("error: failed to connect to proxy.")
        return False
    except socket.timeout:
        print(f"error: proxy didn't reply within {PROXY_TIMEOUT}s.")
        return False
