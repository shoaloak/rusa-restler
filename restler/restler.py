...
from engine.transport_layer.messaging import verify_proxy
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dynamic',
                        help="Enables coverage feedback",
                        action='store_true', default=False, required=False)
    parser.add_argument('--dyn_host',
                        help="host for coverage server",
                        type=str, default=restler_settings.DYN_HOST, required=False)
    parser.add_argument('--dyn_port',
                        help="port for coverage server",
                        type=int, default=restler_settings.DYN_PORT, required=False)
    parser.add_argument('--proxy',
                        help="Route fuzzer traffic through proxy server",
                        action='store_true', default=False, required=False)
    parser.add_argument('--proxy_host',
                         help="Host for proxy server",
                         type=str, default=restler_settings.PROXY_HOST, required=False)
    parser.add_argument('--proxy_port',
                        help="port for proxy server",
                        type=int, default=restler_settings.PROXY_PORT, required=False)
    ...
    # doesn't change anything in our POC
    ...
    if args.proxy and not verify_proxy():
        # problem with proxy
        sys.exit(-1)
    ...
    # Rusa feedback driven fuzzing
    if args.dynamic:
        if args.fuzzing_jobs > 1:
            print("Error: directed multi-threaded fuzzing is currently not supported.")
            sys.exit(-1)
        ZmqClient(args.dyn_host, args.dyn_port)
        if not ZmqClient().connect():
            print("Error: could not connect to the dynamic analysis server.")
            sys.exit(1)
        # queue_thread = ZmqClient(args.dyn_host, args.dyn_port)
        # queue_thread.name = 'Queue'
        # queue_thread.start()
